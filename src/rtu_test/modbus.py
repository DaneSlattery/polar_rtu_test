...
async def connect(tty) -> AsyncModbusSerialClient:
    ...
    client = AsyncModbusSerialClient(
        tty, baudrate=9600, framer=FramerType.RTU, timeout=0.1)
...
async def reset_pulse_count(client: AsyncModbusSerialClient):
    result = await client.write_register(221, 4442)
...
async def set_digi_output_config(client: AsyncModbusSerialClient, output_no: GPIO, value: DigOutDefaultState = DigOutDefaultState.REMEMBER):
    ...
    result = await client.write_register(
        OUT_CONFIG_REGISTER_OFFSET + output_no.value, value.value)
...
async def get_pulse_count(client: AsyncModbusSerialClient, input_no: PulseCounter):
    result = await client.read_holding_registers(PC_REGISTER_OFFSET + input_no.value, count=2)
